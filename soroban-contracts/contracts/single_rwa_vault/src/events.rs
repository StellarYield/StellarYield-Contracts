...
/// Emitted by `propose_emergency_withdraw` — a signer created a new proposal.
pub fn emit_emergency_proposed(e: &Env, proposal_id: u32, proposer: Address, recipient: Address) {
    e.events().publish(
        (symbol_short!("emrg_prop"),),
        (proposal_id, proposer, recipient),
    );
}

/// Emitted by `approve_emergency_withdraw` — a signer approved a proposal.
pub fn emit_emergency_approved(e: &Env, proposal_id: u32, signer: Address, approval_count: u32) {
    e.events().publish(
        (symbol_short!("emrg_aprv"),),
        (proposal_id, signer, approval_count),
    );
}

/// Emitted by `execute_emergency_withdraw` — proposal executed, funds transferred.
pub fn emit_emergency_executed(e: &Env, proposal_id: u32, recipient: Address, amount: i128) {
    e.events().publish(
        (symbol_short!("emrg_exec"),),
